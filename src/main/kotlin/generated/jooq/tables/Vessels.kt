...
open class Vessels(
   ...
   */
  val ID: TableField<VesselsRecord, UUID?> =
    createField(
      DSL.name("id"),
   ...
   */
  val CREATED_AT: TableField<VesselsRecord, OffsetDateTime?> =
    createField(
      DSL.name("created_at"),
  ...
  constructor(path: Table<out Record>, childPath: ForeignKey<out Record, VesselsRecord>?, parentPath: InverseForeignKey<out Record, VesselsRecord>?) : this(
  ...
  ): Vessels = where(DSL.condition(condition))
  ...
  @PlainSQL override fun where(
    @Stringly.SQL condition: String,
    vararg binds: Any?,
  ): Vessels = where(DSL.condition(condition, *binds))
  ...
  ): Vessels = where(DSL.condition(condition, *parts))
