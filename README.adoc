...
In the root folder you also need to copy `_conf.json` into `conf.json` and change the variables as you need. Make sure the `jwt.secret` aligns with the secret you've put in when creating the keystore.
...
==== To run Postgres and SMTP server during local development

----
docker compose up -d
open http://127.0.0.1:5000 # Local SMTP Server
----
