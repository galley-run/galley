...
1. Checks with the platform if it needs to install as controller or controller+worker
...
2. Calls `k0s install controller` (optionally also with flag --enable-worker)
3. Starts K0s
4. Calls k0s to create worker token
5. Shows the next step to join a worker (optional when this controller is a controller+worker)
6. Ask user to execute the following on the worker nodes:
----
...
The Galley Node agent is a small agen t that lives in each node that is provisioned.
